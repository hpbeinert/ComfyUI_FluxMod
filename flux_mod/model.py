...
class FluxLiteMod(nn.Module):
    ...
    @staticmethod
    def distribute_modulations(tensor: torch.Tensor, single_block_count: int = 38, double_blocks_count: int = 8):
        """
        Distributes slices of the tensor into the block_dict as ModulationOut objects.
    ...
    def forward_orig(
        ...
        device = img.device
        dtype = img.dtype
        mod_index_length = 212
        distill_timestep = timestep_embedding(timesteps.detach().clone(), 8).to(device=device, dtype=dtype)
        # guidance = guidance *
        distil_guidance = timestep_embedding(guidance.detach().clone(), 8).to(device=device, dtype=dtype)

        # get all modulation index
        modulation_index = timestep_embedding(torch.arange(mod_index_length), 16).to(device=device, dtype=dtype)
        # we need to broadcast the modulation index here so each batch has all of the index
        modulation_index = modulation_index.unsqueeze(0).repeat(img.shape[0], 1, 1)
        # and we need to broadcast timestep and guidance along too
        timestep_guidance = torch.cat([distill_timestep, distil_guidance], dim=1).unsqueeze(1).repeat(1, mod_index_length, 1)
        ...
        mod_vectors_dict = self.distribute_modulations(mod_vectors, 38, 8)
